...
class Ticket(db.Model):
    ...
    status = db.Column(db.String(20), default='pending')  # pending, open, in_progress, resolved, closed
    ...
    assigned_to_id = db.Column(db.Integer, ForeignKey('employee.id'), nullable=True)
    assigned_to = relationship("Employee", backref="assigned_tickets")
    
    # If the ticket becomes a service order
    service_order_id = db.Column(db.Integer, ForeignKey('service_order.id'), nullable=True)
    service_order = relationship("ServiceOrder", backref=db.backref("ticket", uselist=False))
